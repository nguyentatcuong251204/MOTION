...
def train_epoch(train_loader, model, optimizer, train_meter, cur_epoch, cfg, writer=None, logger=None, device=None):
    """
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        with xp.StepTrace('train_kinetic', step_num=cur_iter):
            ...
            # Chạy trên TPU
                ...
                for key, val in meta.items():
                    if isinstance(val, list):
                        meta[key] = [v.to(device) for v in val]
                    else:
        ...
        # Backward pass
        loss.backward()
        xm.optimizer_step(optimizer)  # TPU-specific optimization step
        xm.mark_step()  # Xác nhận bước tính toán TPU để tránh treo

        logger.info(f"Iteration {cur_iter}: Loss = {loss.item()}")
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    if writer is not None:
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.NUM_GPUS:
                all_preds = [pred for pred in all_preds]
...
def train(cfg):
    ...
    train_loader_temp = loader.construct_loader(cfg, "train")
    val_loader_temp = loader.construct_loader(cfg, "val")
